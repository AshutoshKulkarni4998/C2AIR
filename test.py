...
def test_loop(net, data_loader, num_iter):
    ...
    with torch.no_grad():
        ...
        for rain, norain, name, h, w in test_bar:
            ...
            start = time.time()
            out = model(rain)
            # visualize_featuremaps_batchwise(feature, "./Features/Thin", count)
...
def visualize_featuremaps_batchwise(ylh, filename, itera):
  processed = []
  ...
  for i, feature_map in enumerate(ylh):
      ...
      plt.imsave(filename + '_'+  str(itera)  + '.png', processed[0])
...
if __name__ == '__main__':
    ...
    model = C2AIR(args.expansion_factor).cuda()
